...
var coloring_palette_1 = __importDefault(require("coloring-palette"));
var app = document.getElementById('app');
function getColors(color) {
    var p = coloring_palette_1["default"](color, 'hex');
    var colors = Object.keys(p)
        .filter(function (k) { return !isNaN(k); })
        .slice(0, 5)
        .reverse()
        .map(function (k) { return p[k].color; });
    return colors;
}
var colors = {
    primary: '#0d6efd',
...
function CreateChart(data) {
    ...
    var labels = Object.keys(data.total);
    var datasets = [];
...
function calculateCummulativeTimeSeries(data) {
    var dates = Object.keys(data).reduce(function (obj, key) {
        var region = data[key];
        region.dates.forEach(function (regionDayStat) {
            var date = regionDayStat.date;
            var dayStat = obj[date] = obj[date] || {
                date: date,
                confirmed: 0,
                deaths: 0,
                recovered: 0
    ...
    var datasets = types.map(function (type) {
        return {
            ...
            data: labels.map(function (key) { return dates[key][type]; })
        };
    });
    return { datasets: datasets, labels: labels };
}
function calculateDistinctTimeSeries(data) {
    var datasets = [];
    var labelKeys = {};
    Object.keys(data).forEach(function (key) {
        var region = data[key];
        region.dates.forEach(function (dayStat) { return labelKeys[dayStat.date] = -1; });
    });
    var labels = Object.keys(labelKeys).sort();
    ...
    Object.keys(data).forEach(function (key, index) {
        ...
        types.forEach(function (type) {
            var data = region.dates.reduce(function (arr, dayStat) {
                var expectedIndex = labelKeys[dayStat.date];
...
function calculatePatientZeroTimeSeries(data) {
    var _a = calculateDistinctTimeSeries(data), datasets = _a.datasets, labels = _a.labels;
    var maxSize = datasets
        .filter(function (d) { return d.statType === 'confirmed'; })
        .reduce(function (maxSize, d) {
        var data = d.data;
        var patientZeroIndex = data.findIndex(function (value) { return value > 0; });
        d.data = data.slice(patientZeroIndex);
        return Math.min(maxSize, d.data.length);
    }, Infinity);
    datasets.forEach(function (d) { return d.data = d.data.slice(0, maxSize); });
    labels = labels.slice(0, maxSize).map(function (_, i) { return String(i + 1); });
    return { datasets: datasets, labels: labels };
}
function calculateTotal(data, lastDate, secondToLastDate) {
    return Object.keys(data).reduce(function (obj, key) {
        var dayStat = data[key].dates;
        types.forEach(function (type) {
...
function mergeByCountries(data) {
    var regions = {};
    Object.keys(data.regions).forEach(function (key) {
        var region = data.regions[key];
        var country = region['Country/Region'];
        var resultRegion = regions[country] = regions[country] || {
        ...
        region.dates.forEach(function (dayStat) {
            var date = dayStat.date;
            var resultDayStat = resultRegion.dates[date] = resultRegion.dates[date] || {
                confirmed: 0,
                date: date,
                deaths: 0,
                recovered: 0
    ...
    var regionsWithArray = Object.keys(regions).reduce(function (obj, regionKey) {
        var region = regions[regionKey];
        obj[regionKey] = __assign(__assign({}, region), { dates: Object.keys(region.dates).map(function (date) { return region.dates[date]; }) });
        return obj;
    }, {});
    return {
        date: data.date,
...
var humanReadables = ['k', 'm', 'b'];
function toHumanReadable(value) {
    var suffix = '';
    for (var i = 0; i < humanReadables.length && value > 1000; i++) {
        value = value / 1000;
...
function CheckboxAndLabel(params) {
    ...
    return { node: node, checkbox: checkbox };
}
function Form(dataSource, allData, chart) {
    var dataByCountry = mergeByCountries(allData);
    var data = dataByCountry;
    var selections = getSelections(true);
    function getSelections(selected) {
        return Object.keys(data.regions).reduce(function (obj, key) {
            obj[key] = selected;
            return obj;
        }, {});
    }
    ...
    var selectedStats = (function () {
        var node = document.createElement('div');
        node.className = 'total-stats';
        function createStat(name, fg, bg) {
        ...
        var values = types.reduce(function (obj, type) {
            obj[type] = createStat(type[0].toUpperCase() + type.substring(1) + ':', 'white', colorByType[type]);
            return obj;
        }, {});
        return { node: node, values: values };
    })();
    form.appendChild(selectedStats.node);
    function serializeAndCopyToClipboard(e) {
        e.preventDefault();
    ...
    function calcGrowth(last, secondToLast) {
        return ' (+' + ((last / secondToLast - 1) * 100).toFixed(0) + '%)';
    }
    function updateSelectedStats(data) {
        var stats = calculateTotal(data, lastDate, secondToLastDate);
    ...
    var _a = (function () {
        var allDates = Object.keys(allData.total).sort();
        var size = allDates.length;
        return [allDates[size - 1], allDates[size - 2]];
    }()), lastDate = _a[0], secondToLastDate = _a[1];
    var sort = 'confirmed';
    function rebuildCheckboxes() {
        function getLastValue(region, stat) {
            var value = region.dates[region.dates.length - 1];
            return value && value[stat] ? value[stat] : 0;
        }
        checkboxes.length = 0;
        countries.innerHTML = '';
        var divs = Object.keys(data.regions)
            .sort(function (key1, key2) {
            if (sort === 'name') {
                return key1 < key2 ? -1 : 1;
            }
            var r1 = data.regions[key1];
            var r2 = data.regions[key2];
            return getLastValue(r2, sort) - getLastValue(r1, sort);
        })
            .map(function (key) {
            var casesKey = sort === 'name' ? 'confirmed' : sort;
            var cases = ' ' + toHumanReadable(getLastValue(data.regions[key], casesKey));
            var _a = CheckboxAndLabel({
                id: key,
                className: 'country',
                label: key + cases,
            ...
            }), node = _a.node, checkbox = _a.checkbox;
            checkboxes.push(checkbox);
            return node;
        });
        divs.forEach(function (div) { return countries.appendChild(div); });
    }
    rebuildCheckboxes();
    ...
    function update() {
        var regions = Object
            .keys(data.regions)
            .filter(function (key) { return selections[key]; })
            .reduce(function (obj, key) {
            obj[key] = data.regions[key];
            return obj;
        }, {});
        if (cummulative.checkbox.checked) {
            var _a = calculateCummulativeTimeSeries(regions), datasets = _a.datasets, labels = _a.labels;
            chart.data.labels = labels;
            chart.data.datasets = datasets;
        }
        else {
            var _b = patientZero.checkbox.checked
                ? calculatePatientZeroTimeSeries(regions)
                : calculateDistinctTimeSeries(regions), datasets = _b.datasets, labels = _b.labels;
            chart.data.labels = labels;
            chart.data.datasets = datasets;
        }
    ...
    selectAllButton.addEventListener('click', function (e) {
        ...
        checkboxes.forEach(function (c) { return c.checked = true; });
        update();
    });
    ...
    unselectAllButton.addEventListener('click', function (e) {
        ...
        checkboxes.forEach(function (c) { return c.checked = false; });
        update();
    });
    buttons.className = 'buttons';
    buttons.appendChild(selectAllButton);
    buttons.appendChild(unselectAllButton);
    var cummulative = CheckboxAndLabel({
        id: 'cummulative',
    ...
    var patientZero = CheckboxAndLabel({
        id: 'patientZero',
    ...
    var perCountry = CheckboxAndLabel({
        id: 'merge',
    ...
    sortSelect.id = 'sort';
    var sortOptions = ['name', 'confirmed', 'recovered', 'deaths'];
    sortOptions.forEach(function (sortId) {
    ...
    var scaleOptions = ['linear', 'logarithmic'];
    scaleOptions.forEach(function (scaleId) {
        var option = document.createElement('option');
    ...
    function serialize() {
        var values = {
            ...
            checkboxes: checkboxes.filter(function (c) { return c.checked; }).reduce(function (obj, c) {
                obj[c.id] = true;
                return obj;
            }, {}),
            ...
            hiddenCharts: chart.data.datasets
                .map(function (d, i) { return ({ hidden: chart.getDatasetMeta(i).hidden, index: i }); })
    ...
    function deserialize() {
        ...
        try {
            var values_1 = JSON.parse(decodeURIComponent(hash));
            checkboxes.forEach(function (checkbox) {
                if (values_1.checkboxes[checkbox.id]) {
            ...
            cummulative.checkbox.checked = values_1.cummulative;
            cummulative.checkbox.checked = values_1.cummulative;
            if (values_1.cummulative) {
                patientZero.checkbox.setAttribute('disabled', '');
            }
            else {
                patientZero.checkbox.removeAttribute('disabled');
            }
            patientZero.checkbox.checked = values_1.patientZero;
            perCountry.checkbox.checked = values_1.perCountry;
            sort = values_1.sort;
            scale = values_1.scale;
            Array.from(sortSelect.options).forEach(function (option) {
                option.selected = option.value === sort;
            });
            rebuildCheckboxes();
            hiddenCharts = values_1.hiddenCharts || hiddenCharts;
        }
        catch (err) {
            console.error('Error deserializing:', err);
        }
    }
    var onLegendClick = chart.options.legend.onClick;
    chart.options.legend.onClick = function (e, legendLabelItem) {
        onLegendClick.call(this, e, legendLabelItem);
    };
    deserialize();
    update();
    hiddenCharts.forEach(function (index) {
        var d = chart.data.datasets[index];
...
function createDataSourceSelector(dataSource) {
...
function loadAndRender(dataSource) {
    return __awaiter(this, void 0, void 0, function () {
        var result, data, _a, chart, canvas;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    app.innerHTML = 'Loading ' + dataSource;
                    return [4 /*yield*/, fetch(dataURLs[dataSource])];
                case 1:
                    result = _b.sent();
                ...
                case 2:
                    data = _b.sent();
                    app.innerHTML = '';
                    _a = CreateChart(data), chart = _a.chart, canvas = _a.canvas;
...
function getDataSource() {
    var dataSource = 'csse';
    try {
        var result = JSON.parse(decodeURIComponent(location.hash.substring(1)));
...
loadAndRender(getDataSource())["catch"](function (err) { return app.innerHTML = 'An error occurred: ' + err.message; });

},{"chart.js":13,"coloring-palette":15}],2:[function(require,module,exports){
"use strict";
...
},{"./conversion":2,"./css-color-names":3,"./util":11}],5:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
},{"./index":6,"./util":11}],6:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var conversion_1 = require("./conversion");
var css_color_names_1 = require("./css-color-names");
...
},{"./conversion":2,"./css-color-names":3,"./format-input":4,"./util":11}],7:[function(require,module,exports){
"use strict";
function __export(m) {
...
},{"./css-color-names":3,"./format-input":4,"./from-ratio":5,"./index":6,"./random":8,"./readability":9,"./to-ms-filter":10}],8:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var index_1 = require("./index");
...
},{"./conversion":2,"./index":6}],11:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
},{"moment":16}],14:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
exports.linear = bezier_easing_1.default(0.5, 0.5, 0.5, 0.5);
exports.easeInCubic = bezier_easing_1.default(0.55, 0.055, 0.675, 0.19);
exports.easeOutCubic = bezier_easing_1.default(0.215, 0.61, 0.355, 1);
exports.easeInOutCubic = bezier_easing_1.default(0.645, 0.045, 0.355, 1);
exports.easeInSine = bezier_easing_1.default(0.47, 0, 0.745, 0.715);
exports.easeOutSine = bezier_easing_1.default(0.39, 0.575, 0.565, 1);
exports.easeInOutSine = bezier_easing_1.default(0.445, 0.05, 0.55, 0.95);
exports.easeInQuad = bezier_easing_1.default(0.55, 0.085, 0.68, 0.53);
exports.easeOutQuad = bezier_easing_1.default(0.25, 0.46, 0.45, 0.94);
exports.easeInOutQuad = bezier_easing_1.default(0.455, 0.03, 0.515, 0.955);
exports.easeInQuart = bezier_easing_1.default(0.895, 0.03, 0.685, 0.22);
exports.easeOutQuart = bezier_easing_1.default(0.165, 0.84, 0.44, 1);
exports.easeInOutQuart = bezier_easing_1.default(0.77, 0, 0.175, 1);
exports.easeInCirc = bezier_easing_1.default(0.6, 0.04, 0.98, 0.335);
exports.easeOutCirc = bezier_easing_1.default(0.075, 0.82, 0.165, 1);
exports.easeInOutCirc = bezier_easing_1.default(0.785, 0.135, 0.15, 0.86);
exports.easeInQuint = bezier_easing_1.default(0.755, 0.05, 0.855, 0.06);
exports.easeOutQuint = bezier_easing_1.default(0.23, 1, 0.32, 1);
exports.easeInOutQuint = bezier_easing_1.default(0.86, 0, 0.07, 1);
exports.easeInExpo = bezier_easing_1.default(0.95, 0.05, 0.795, 0.035);
exports.easeOutExpo = bezier_easing_1.default(0.19, 1, 0.22, 1);
exports.easeInOutExpo = bezier_easing_1.default(1, 0, 0, 1);
exports.easeInBack = bezier_easing_1.default(0.6, -0.28, 0.735, 0.045);
exports.easeOutBack = bezier_easing_1.default(0.175, 0.885, 0.32, 1.275);
exports.easeInOutBack = bezier_easing_1.default(0.68, -0.55, 0.265, 1.55);
exports.muiHueCurve = bezier_easing_1.default(0.34, 0.1, 0.68, 1);
exports.muiSatCurve = bezier_easing_1.default(0.48, 0.78, 0.31, 0.99);
exports.muiValCurve = bezier_easing_1.default(0.4, 0.55, 0.66, 1);

},{"bezier-easing":12,"tslib":17}],15:[function(require,module,exports){
"use strict";
/*
...
Object.defineProperty(exports, "__esModule", { value: true });
const tslib_1 = require("tslib");
const tinycolor_1 = tslib_1.__importDefault(require("@ctrl/tinycolor"));
...
function contrastText(color) {
    return tinycolor_1.default(color).getLuminance() < 1 / 3 ? tinycolor_1.default('#FFF') : tinycolor_1.default('#000');
}
exports.contrastText = contrastText;
function altColor(color, alt) {
    const { h, s, v } = tinycolor_1.default(color).toHsv();
    const { hAdj, sAdj, vAdj } = altColorAdjs[alt];
    return tinycolor_1.default({ h: (h + hAdj) % 360, s: atMost100(s * sAdj), v: atMost100(v * vAdj) });
}
...
function generateShades({ hueStart, hueEnd, hueCurve = curves_1.easeInQuad, satStart, satEnd, satCurve = curves_1.easeOutQuad, satRate = 100, valStart, valEnd, valCurve = curves_1.easeOutQuad, steps = 10, format = null, }) {
    ...
    for (let index = 0; index < steps; index++) {
        ...
        const color = tinycolor_1.default({
...
function generateMaterialUIPalette({ hueStart, hueEnd, hueCurve = curves_1.muiHueCurve, satStart, satEnd, satCurve = curves_1.muiSatCurve, satRate = 100, valStart, valEnd, valCurve = curves_1.muiValCurve, format = 'hex', }) {
...
exports.generateMaterialUIPalette = generateMaterialUIPalette;
function coloringPalette(color, format = 'hex') {
    const defaultColor = tinycolor_1.default(color);
...
exports.default = coloringPalette;

},{"./curves":14,"@ctrl/tinycolor":7,"tslib":17}],16:[function(require,module,exports){
//! moment.js
